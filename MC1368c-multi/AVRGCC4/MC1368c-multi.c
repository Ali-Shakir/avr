...
// передача начального адреса, с которого данные помещаются в регистр данных LCD
LCD_write_byte(0,6);//0b000000
//отправляем 16 раз в индикатор пустой байт, чтоб погасить все LCD сегменты и мнемонические символы, то есть очищаем дисплей. 
//Счетчик адреса в индикаторе автоматиески увеличивает значение на 1 за каждые 4 бита. 
for (i=0; i<16;i++)
{
...
// команда на разрешение записи в регистр данных LCD
LCD_write_byte(0x5,3);//0b101
LCD_write_byte((pos<<1),6);//вычисляем начальный адрес ячейки регистра относительно позиции символа на дисплее и записываем его в дисплей. 
...
{         
	...
	if(ascii_conv>=0x31){ascii_conv=ascii_conv-0x27;} // дешифруем буквы для HEX (radix 16)
...
// команда на разрешение записи в регистр данных LCD
LCD_write_byte(0x5,3);//0b101 
LCD_write_byte((address<<1),6);//вычисляем начальный адрес ячейки регистра, относительно позиции мнемосимвола на дисплее, и записываем его в дисплей.

LCD_write_byte(lcd_buffer[symb_pos[symbol]],8); //отправляем указанный байт с измененным 7 битом из буфера в дисплей, чтоб зажечь или погасить мнемосимвол. 
